...
class PersonalGoal_9Test {
    ...
    PersonalGoal_9 goal = new PersonalGoal_9();
    Spot[][] spots = goal.getSpotGrid();

    @BeforeEach
    void setUp(){
        goal = new PersonalGoal_9();
    }

    //Test 1: check that the tiles are in the right position
    @Test
    void checkCorrectPosition(){
        assert spots[2][2].getTile().getColor().equals(Color.GREEN);
        assert spots[4][4].getTile().getColor().equals(Color.PINK);
        assert spots[0][2].getTile().getColor().equals(Color.YELLOW);
        assert spots[4][1].getTile().getColor().equals(Color.AZURE);
        assert spots[3][4].getTile().getColor().equals(Color.WHITE);
        assert spots[5][0].getTile().getColor().equals(Color.BLUE);
    }
    ...
    void emptyCheckPosition(){
        for(int i=0; i<6; i++){
            for(int j=0; j<5; j++){
                if((i==2 && j==2) || (i==4 && j==4) || (i==0 && j==2) || (i==4 && j==1) || (i==3 && j==4)
                        || (i==5 && j==0)){
                    continue;
                }
